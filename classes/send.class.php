<?php
  ...
  class Send extends dbQuery
  {
      ...
      */
      ...
      {
          $this->connect();
          $exists = !empty($this->query("SELECT `id` FROM `fb_users` WHERE `fb_id`=?", $userInfo['id']));

          if ($exists) {
              $this->query(
                  "UPDATE `fb_users` SET `first_name`=?, `locale`=? WHERE `fb_id`=?",
                  $userInfo['first_name'],
                  $userInfo['locale'],
                  $userInfo['id']
              );
      ...
      {
          $this->connect();
          $this->query("UPDATE `fb_users` SET `is_subscribed`=0, `fb_token`=null WHERE `fb_id`=?", $ID);
      }
  }
